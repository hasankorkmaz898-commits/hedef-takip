...
const PRO_TEMPLATES = [
  {
    icon:'🏃', name:'12 Haftalık Maraton Hazırlığı', weekCount:12, bufferDays:[5],
    weeks: Array.from({length:12},(_,i)=>({
      name: i<4?`Baz Antrenman - ${i+1}. Hafta`: i<8?`Tempo Artışı - ${i-3}. Hafta`:`Yarış Hazırlığı - ${i-7}. Hafta`,
      days: [
        {dow:1,enabled:true,tasks:['Uzun koşu','Esneme']},
        {dow:2,enabled:true,tasks:['Tempo koşusu']},
        {dow:3,enabled:true,tasks:['Dinlenme koşusu','Kuvvet antrenmanı']},
        {dow:4,enabled:true,tasks:['Interval antrenmanı']},
        {dow:5,enabled:true,tasks:['Kısa koşu','Beslenme takibi']},
        {dow:6,enabled:false,tasks:['']},
        {dow:0,enabled:false,tasks:['']},
      ]
    }))
  },
  {
    icon:'🗣️', name:'6 Aylık İngilizce Programı', weekCount:24, bufferDays:[0],
    weeks: Array.from({length:24},(_,i)=>({
      name: i<8?`Temel Seviye - ${i+1}. Hafta`: i<16?`Orta Seviye - ${i-7}. Hafta`:`İleri Seviye - ${i-15}. Hafta`,
      days: [
        {dow:1,enabled:true,tasks:['30 dk kelime çalışması','10 dk konuşma pratiği']},
        {dow:2,enabled:true,tasks:['Gramer alıştırmaları']},
        {dow:3,enabled:true,tasks:['30 dk kelime çalışması','Podcast dinle']},
        {dow:4,enabled:true,tasks:['Yazma pratiği']},
        {dow:5,enabled:true,tasks:['30 dk kelime','Haftalık tekrar']},
        {dow:6,enabled:true,tasks:['Film/dizi izle (İngilizce)']},
        {dow:0,enabled:false,tasks:['']},
      ]
    }))
  },
  {
    icon:'💪', name:'4 Haftalık Detoks Programı', weekCount:4, bufferDays:[0],
    weeks: Array.from({length:4},(_,i)=>({
      name: ['Hazırlık','Yoğunlaşma','Derin Temizlik','Pekiştirme'][i]+` - ${i+1}. Hafta`,
      days: [
        {dow:1,enabled:true,tasks:['Sabah suyu 1L','Şekersiz beslen','30 dk egzersiz']},
        {dow:2,enabled:true,tasks:['Meditasyon 10 dk','Yeşil smoothie']},
        {dow:3,enabled:true,tasks:['Sabah suyu 1L','Şekersiz beslen','30 dk yürüyüş']},
        {dow:4,enabled:true,tasks:['Meditasyon 10 dk','Erken uyku (22:00)']},
        {dow:5,enabled:true,tasks:['Sabah suyu 1L','Haftalık değerlendirme']},
        {dow:6,enabled:true,tasks:['Aktif dinlenme','Doğa yürüyüşü']},
        {dow:0,enabled:true,tasks:['Haftalık hazırlık','Yemek planla']},
      ]
    }))
  },
  {
    icon:'🧘', name:'8 Haftalık Mindfulness', weekCount:8, bufferDays:[0],
    weeks: Array.from({length:8},(_,i)=>({
      name: `${i+1}. Hafta · ${['Farkındalık','Nefes','Beden Taraması','Duygu Yönetimi','Odaklanma','Kabul','Şükran','Entegrasyon'][i]}`,
      days: [
        {dow:1,enabled:true,tasks:['Sabah meditasyonu 10 dk','Günlük yaz']},
        {dow:2,enabled:true,tasks:['Nefes egzersizi 5 dk']},
        {dow:3,enabled:true,tasks:['Sabah meditasyonu 10 dk','Şükran listesi']},
        {dow:4,enabled:true,tasks:['Yürüyüş meditasyonu']},
        {dow:5,enabled:true,tasks:['Sabah meditasyonu','Haftalık özet']},
        {dow:6,enabled:false,tasks:['']},
        {dow:0,enabled:false,tasks:['']},
      ]
    }))
...
export default function ProfessionalPlanModal({ user, onClose, onSaved }) {
  const [view,      setView]    = useState('start') // start | setup | weeks
  const [planName,  setPlanName]= useState('')
  const [weekCount, setWkCount] = useState(4)
  const [bufferDay, setBufferDay]= useState(null)   // telafi günü: dow veya null
  ...
  function applyTemplate(tpl) {
    ...
    if (tpl.bufferDays?.[0]!=null) setBufferDay(tpl.bufferDays[0])
    setView('weeks')
  }
  ...
  async function handleSave() {
    ...
    const totalDays = weekCount*7 + (bufferDay!=null?weekCount:0)
    setSaving(true)
    try {
      ...
      weeks.forEach((week,wi)=>{
        ...
        // Telafi günü görevi (sistem otomatik)
        if (bufferDay!=null) {
          taskRows.push({
            goal_id:    goal.id,
            name:       '⚡ Telafi Günü',
            order_index:99,
            active_days:[bufferDay],
            week_number:wi+1,
            week_name:  week.name,
            is_buffer:  true,
          })
        ...
        <div style={{background:'rgba(251,191,36,0.07)',border:'1.5px solid rgba(251,191,36,0.25)',borderRadius:16,padding:14,marginBottom:14}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:'var(--text3)'}}>Her haftaya otomatik 1 telafi günü ekle — kaçırdığın görevleri taşı</div>
            </div>
            <button onClick={()=>setBufferDay(bufferDay!=null?null:0)} style={{marginLeft:'auto',padding:'6px 12px',borderRadius:10,border:`1.5px solid ${bufferDay!=null?'rgba(251,191,36,0.5)':'var(--border)'}`,background:bufferDay!=null?'rgba(251,191,36,0.15)':'var(--surface2)',color:bufferDay!=null?'var(--mid)':'var(--text3)',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
              {bufferDay!=null?'Açık':'Kapalı'}
            </button>
          </div>
          {bufferDay!=null && (
            <div>
              <span style={s.label}>Telafi günü hangi gün?</span>
      ...
      <div style={s.sheet}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <div>
            ...
            <div style={{fontSize:16,fontWeight:800,color:'var(--text)',marginTop:4}}>{planName||'Plan Adı'}</div>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',color:'var(--text3)',fontSize:20,cursor:'pointer'}}>✕</button>
        </div>
        ...
        {wk && (
          <div style={{background:'var(--surface2)',borderRadius:18,padding:16,marginBottom:14}}>
            <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:14}}>
              <input value={wk.name} onChange={e=>updateWeekName(activeWk,e.target.value)} placeholder="Hafta adı" style={{...s.input,fontSize:14,fontWeight:700}}/>
            ...
            {((activeWk+1)%4===0)&&(
              <div style={{background:'rgba(251,191,36,0.07)',border:'1.5px solid rgba(251,191,36,0.25)',borderRadius:12,padding:'8px 12px',marginBottom:12,fontSize:11,color:'var(--mid)',display:'flex',gap:6,alignItems:'center'}}>
                <span>🏆</span> Bu hafta bir kilometre taşı haftası
              </div>
            )}
            ...
            {wk.days.filter(d=>d.enabled).length===0?(
              ...
              <div key={day.dow} style={{marginBottom:14}}>
                ...
                {day.tasks.map((task,ti)=>(
                  <div key={ti} style={{display:'flex',gap:7,alignItems:'center',marginBottom:7}}>
                    <input value={task} onChange={e=>updateTask(activeWk,day.dow,ti,e.target.value)} placeholder={`${DOW_FULL[day.dow]} görevi ${ti+1}`} style={{...s.input,flex:1}}/>
