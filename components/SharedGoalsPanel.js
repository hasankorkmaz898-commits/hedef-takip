...
function dayScore(tasks, logs, ds, userId) {
  ...
  return tasks.reduce((s,t) => {
    const l = dl.find(x => x.task_id === t.id)
    return s + (l ? Q[l.quality] : 0)
  }, 0) / tasks.length
}
...
const css = {
  ...
  taskItem: (q) => ({ background: q?({good:'rgba(52,211,153,0.1)',mid:'rgba(251,191,36,0.1)',bad:'rgba(248,113,113,0.1)'}[q]):'#242830', border:`1px solid ${q?({good:'rgba(52,211,153,0.3)',mid:'rgba(251,191,36,0.3)',bad:'rgba(248,113,113,0.3)'}[q]):'#2e3340'}`, borderRadius:10, overflow:'hidden', marginBottom:7 }),
}
...
export default function SharedGoalsPanel({ user, initialFriend, onClose }) {
  ...
  async function loadAll() {
    // Load shared goals where user is member or creator
    ...
    const createdIds = (createdGoals || []).map(g => g.id)
    const allIds = [...new Set([...memberGoalIds, ...createdIds])]
    ...
    // Collect all unique user ids for profiles
    ...
    if (fs) {
      setFriends(fs.map(f => f.requester_id === user.id ? f.receiver : f.requester))
    }
  }
  ...
  async function createSharedGoal() {
    ...
    const { data: g } = await supabase.from('shared_goals').insert({
    ...
    await supabase.from('shared_goal_members').insert([
      { goal_id: g.id, user_id: user.id },
      { goal_id: g.id, user_id: selectedFriend.id },
    ])
      ...
      <div style={css.panel}>
        ...
        <div style={css.header}>
          ...
          <div>
            ...
            <div style={{ fontSize:11,color:'#5c6475' }}>{goals.length} aktif hedef</div>
          </div>
          <button onClick={() => setShowCreate(true)} style={{ ...css.btn('primary'), marginLeft:'auto', padding:'8px 14px', fontSize:12 }}>+ Yeni</button>
        </div>
        ...
        <div style={{ padding:'16px 20px' }}>
          ...
          {showCreate && (
            <div style={{ ...css.card, border:'1px solid rgba(99,102,241,0.4)', marginBottom:20 }}>
              ...
              <div style={{ marginBottom:12 }}>
                ...
                {friends.length === 0
                  ? <div style={{ fontSize:13, color:'#f87171' }}>Önce arkadaş eklemelisin</div>
                  : <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                    {friends.map(f => (
                      <div key={f.id} onClick={() => setSelectedFriend(f)} style={{ padding:'7px 12px', borderRadius:99, border:`1.5px solid ${selectedFriend?.id===f.id?'#6366f1':'#2e3340'}`, background:selectedFriend?.id===f.id?'rgba(99,102,241,0.15)':'transparent', color:selectedFriend?.id===f.id?'#a5b4fc':'#9aa0b0', fontSize:13, cursor:'pointer' }}>
                        {f.display_name}
                      </div>
                    ))}
                  </div>
          ...
          {goals.length === 0 && !showCreate && (
            <div style={{ textAlign:'center', padding:'50px 20px', color:'#5c6475' }}>
              ...
              <div style={{ fontSize:13 }}>Arkadaşlarınla birlikte hedef belirle</div>
            </div>
          )}
              ...
              <div key={goal.id} style={css.card}>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:4 }}>
                    ...
                    <div style={{ fontSize:10, color:'#5c6475', textAlign:'right' }}>{el}/{goal.total_days} gün<br/>{rem} kaldı</div>
                  </div>
                  {/* Members avatars */}
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:8 }}>
                    {memberScores.map(m => (
                      <div key={m.user_id} style={{ display:'flex', alignItems:'center', gap:5, background:'#111318', border:'1px solid #2e3340', borderRadius:99, padding:'4px 10px' }}>
                        ...
                        <span style={{ fontSize:12, color: m.isMe ? '#a5b4fc' : '#9aa0b0' }}>{m.isMe ? 'Sen' : m.profile?.display_name}</span>
                        <span style={{ fontSize:12, fontWeight:700, color: m.op>=70?'#34d399':m.op>=35?'#6366f1':'#f87171' }}>{m.op}%</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {tab === 'mine' && (
                  <div>
                    <div style={{ ...css.sectionTitle, marginBottom:8 }}>Bugünün Görevleri · {myLogs.length}/{gtasks.length}</div>
                    {gtasks.map(t => {
                      ...
                      return (
                        <div key={t.id} style={{ background:q?qBg[q]:'#242830', border:`1px solid ${q?qBorder[q]:'#2e3340'}`, borderRadius:10, overflow:'hidden', marginBottom:7 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 12px', cursor:'pointer' }} onClick={()=>toggleSharedTask(goal.id, t.id)}>
                            <div style={{ width:22,height:22,borderRadius:6,border:`2px solid ${q?`var(--${q})`:'#3a4050'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,flexShrink:0, background:q?qBg[q]:'transparent', color:q?{good:'#34d399',mid:'#fbbf24',bad:'#f87171'}[q]:'transparent' }}>{q?QSym[q]:''}</div>
                            <div style={{ flex:1, fontSize:13, textDecoration:q?'line-through':'none', color:q?'#5c6475':'#e2e6f0' }}>{t.name}</div>
                            {q && <span style={{ fontSize:10, fontWeight:600, color:{good:'#34d399',mid:'#fbbf24',bad:'#f87171'}[q], background:qBg[q], padding:'2px 7px', borderRadius:99 }}>{QL[q]}</span>}
                          </div>
                          {q && (
                            <div style={{ display:'flex', gap:5, padding:'0 12px 10px' }}>
                              {['good','mid','bad'].map(qv => (
                                <button key={qv} onClick={()=>setSharedQuality(goal.id, t.id, qv)} style={{ flex:1, padding:'6px', borderRadius:7, border:`1.5px solid ${q===qv?{good:'#34d399',mid:'#fbbf24',bad:'#f87171'}[qv]:'#2e3340'}`, background:q===qv?qBg[qv]:'transparent', color:q===qv?{good:'#34d399',mid:'#fbbf24',bad:'#f87171'}[qv]:'#5c6475', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>{QL[qv]}</button>
                              ))}
                            </div>
                          )}
                        </div>
                ...
                {tab === 'all' && (
                  <div>
                    <div style={{ ...css.sectionTitle, marginBottom:10 }}>Üye Karşılaştırması</div>
                    {memberScores.map(m => {
                    ...
                    {/* Per-task breakdown */}
                    <div style={{ ...css.sectionTitle, marginTop:16, marginBottom:8 }}>Bugün Görev Durumu</div>
                    {gtasks.map(t => (
                      <div key={t.id} style={{ background:'#111318', border:'1px solid #2e3340', borderRadius:10, padding:'10px 12px', marginBottom:7 }}>
                        <div style={{ fontSize:13, color:'#e2e6f0', marginBottom:8 }}>{t.name}</div>
                        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                          {memberScores.map(m => {
                            const l = glogs.find(x => x.task_id === t.id && x.log_date === todayStr() && x.user_id === m.user_id)
                            const q = l?.quality
                            const colors = {good:'#34d399',mid:'#fbbf24',bad:'#f87171'}
                            return (
                              <div key={m.user_id} style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 9px', borderRadius:99, background:q?({good:'rgba(52,211,153,0.1)',mid:'rgba(251,191,36,0.1)',bad:'rgba(248,113,113,0.1)'}[q]):'#242830', border:`1px solid ${q?(colors[q]+'55'):'#2e3340'}` }}>
                                <span style={{ fontSize:11, color:q?colors[q]:'#5c6475' }}>{m.isMe?'Sen':m.profile?.display_name}</span>
                                {q ? <span style={{ fontSize:11, fontWeight:700, color:colors[q] }}>{QSym[q]} {QL[q]}</span> : <span style={{ fontSize:11, color:'#5c6475' }}>—</span>}
                              </div>
                            )
                          })}
                        </div>
